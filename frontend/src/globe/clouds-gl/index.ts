 ...
 * cloud-draw point — preserving paint order (labels/storms stay on top).
 *
...
// dense middle, rounded base, wispy top
float heightProfile(float h) {
  float base = clamp(remap(h, 0.0, 0.12, 0.0, 1.0), 0.0, 1.0);
  float top = clamp(remap(h, 0.45, 1.0, 1.0, 0.0), 0.0, 1.0);
  return base * top;
}
float coverage(float lon, float lat) {
...
float sampleDensity(vec3 wp, float lon, float lat, float hf, vec3 windOff) {
  ...
  float d = remap(base * heightProfile(hf), 1.0 - cov, 1.0, 0.0, 1.0);
  if (d <= 0.0) return 0.0;
...
void main() {
  ...
  for (int i = 0; i < STEPS; i++) {
    ...
    float dens = (uSolid == 1) ? heightProfile(hf) : sampleDensity(wp, lon, lat, hf, windOff);
    ...
    for (int s = 1; s <= LIGHT_STEPS; s++) {
      ...
      lightDens += (uSolid == 1) ? heightProfile(lhf)
...
export class CloudGL {
  ...
  private async loadNoise() {
    try {
      const [baseBuf, detailBuf] = await Promise.all([
